...
def load(norm, init, length, run):
    #fn = '-'.join([guide, reparam, length, run]) + '.json'
    #path = 'rg/exp/cont-soft-eq-results/'
    fn = '-'.join(['rnn', norm, init, length, run]) + '.json'
    path = 'rg/exp/cont-soft-eq-norm-and-init-results/'
    with open(path + fn) as f:
        return thin(json.load(f), 500)

#guides = ['rnn']#'rnn gru lstm'.split(' ')
#reparams = 'true false'.split(' ')

norms = 'none layer weight'.split(' ')
inits = 'rand xavier'.split(' ')
...
for norm in norms:
    for length in lengths:
        ix += 1
        plt.subplot(len(norms), len(lengths), ix)
        plt.title('%s norm - %s' % ('no' if norm == 'none' else norm, length))
        plt.ylim(-5, -2)
        ...
        for init in inits:
            c = 'g' if init == 'xavier' else 'orange'


            for run in runs:
                data = load(norm, init, length, run)
                plt.plot(data, c=c)
...
plt.suptitle('soft eq dependency between 2 continuous variables\n(rnn guide, adam .001, 10 samples, green=xavier init, orange=randn init)')
plt.savefig('cont-soft-eq-norm-and-init.png')
plt.show()
